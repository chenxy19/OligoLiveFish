...
def load_fov_with_metadata(nd2_path: Path):
    """Load nd2 → (T, C, Y, X) uint16 plus physical metadata for TIFF export."""
    with ND2File(nd2_path) as f:
        ...
        try:
            vx = f.voxel_size()
            px_um_x, px_um_y = float(vx.x), float(vx.y)
        except Exception:
            px_um_x = px_um_y = 1.0  # fallback — will warn below
        ...
        try:
            for loop in f.experiment:
                if getattr(loop, 'type', '').lower().startswith('time'):
                    # period is ms in nd2 v0.10+; older versions use periodMs
    ...
    # Per-channel LUTs (same logic as before)
...
def process_json(json_path: Path):
    ...
    for crop_info in crops:
        ...
        # ImageJ 'Labels' must have T*C entries — one per plane, in T-major order
        labels = [meta['chan_names'][c] for _ in range(T) for c in range(C)]
        ...
        tifffile.imwrite(
            ...
            crop,
            imagej=True,
            resolution=(xres, yres),  # top-level, not inside metadata
...
def main():
    ...
    json_files = sorted(root.rglob("*_crops.json"))  # recursive: searches entire subtree, not just top level
    if not json_files:
