...
V210_SCRIPT  = HERE / 'auto_roi_for_published_v2.12.py'
SPT_SCRIPT   = HERE / 'run_pipeline_v2.py'
...
def main():
    if len(sys.argv) != 2:
        print("Usage: python3 run_full_pipeline_v2.12.py <try_analysis_dir>")
        sys.exit(1)
    ...
    log_path = analysis_dir / 'log_trajectory_v2.12.txt'
    with open(log_path, 'w') as log_file:
        ...
        try:
            ...
            # Step 1 — reference trajectories (v2.11 with joint seeding)
            run(['python3', str(V210_SCRIPT), str(nucleus_path)])

            # Step 2 — MATLAB SPT trajectories
