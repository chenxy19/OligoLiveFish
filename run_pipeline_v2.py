#!/usr/bin/env python3
"""
run_pipeline_v2.py — Run SPT directly on full-field channel TIFFs, no ROI segmentation.

Usage:
    python3 run_pipeline_v2.py /path/to/input_dir/
...
Requirements: Pillow, numpy, scipy  (pip install -r requirements.txt)
"""
...
MATLAB_BIN    = Path('/Applications/MATLAB_R2026a.app/bin/matlab')
...
def run_spt(tifs: dict, frame_rate: float, pixl_um: float, out_dir: Path):
    for ch, tif in tifs.items():
        ...
        matlab_cmd = (
            f"addpath('{SPT_DIR}'); "
            f"addpath('{SPT_TOOLS_DIR}'); "
            f"addpath('{HERE}'); "
            f"spt_batch('{tif}', {frame_rate}, {pixl_um}, '{out_dir}')"
        )
        result = subprocess.run(
            [str(MATLAB_BIN), '-batch', matlab_cmd],
...
def main():
    if len(sys.argv) != 2:
        print('Usage: python3 run_pipeline_v2.py /path/to/input_dir/')
        sys.exit(1)
